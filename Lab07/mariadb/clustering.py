...
def main():
    ...
    # Could come in handy https://media.readthedocs.org/pdf/kazoo/1.1/kazoo.pdf
    # Format inside values: {IP1},{IP2},...,{IPn}
    ...
    try:
        ...
        # Can you send me a note as to why zk encodes their values?  Took awhile to figure out
        # that I had to decode them and encode them at utf-8
        IP = zk.get(CLUSTER_NODES)[0].decode(coder)
        zk.set(CLUSTER_NODES, '{},{}'.format(IP, get_ip_address()).encode(coder)) # {IP},{newIP}
        if (zk.exists):
            start_node(IP)
        else:
            bootstrap_cluster()
